...
const PetSystem: React.FC = () => {
  ...
  const petFoods: PetFood[] = [
    { id: 'basic_food', name: '기본 사료', icon: '🥫', hungerRestore: 20, happinessBonus: 5, cost: 50 },
    { id: 'premium_food', name: '프리미엄 사료', icon: '🍖', hungerRestore: 40, happinessBonus: 15, cost: 200 },
    { id: 'treat', name: '특별 간식', icon: '🍪', hungerRestore: 10, happinessBonus: 30, cost: 150 },
    { id: 'magic_food', name: '마법 사료', icon: '✨', hungerRestore: 50, happinessBonus: 25, cost: 500 }
  ];

  const trainingActivities = [
    { id: 'attack_training', name: '공격 훈련', stat: 'attack', cost: 200, expGain: 20, icon: '⚔️' },
    { id: 'defense_training', name: '방어 훈련', stat: 'defense', cost: 200, expGain: 20, icon: '🛡️' },
    { id: 'speed_training', name: '속도 훈련', stat: 'speed', cost: 200, expGain: 20, icon: '💨' },
    { id: 'intelligence_training', name: '지능 훈련', stat: 'intelligence', cost: 200, expGain: 20, icon: '🧠' }
  ];
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setMyPets(prev => {
        ...
        // Save updated pets
        localStorage.setItem('myPets', JSON.stringify(updated));
        return updated;
      });
    }, 30000); // 30초마다
  ...
  const adoptPet = (pet: Pet) => {
    ...
    // Prevent adopting more than 6 pets
    if (myPets.length >= 6) {
      alert('최대 6마리까지만 입양할 수 있습니다!');
      return;
    }
  ...
  const feedPet = (pet: Pet, food: PetFood) => {
    ...
    setMyPets(updatedPets);
    updatePlayer({ money: (player.money || 0) - food.cost });
    soundManager.playSuccessSound();
  };

  const trainPet = (pet: Pet, activity: any) => {
    if ((player.money || 0) < activity.cost) {
      alert('골드가 부족합니다!');
      return;
    }
    ...
    setMyPets(updatedPets);
    updatePlayer({ money: (player.money || 0) - activity.cost });
    soundManager.playSuccessSound();
  };
